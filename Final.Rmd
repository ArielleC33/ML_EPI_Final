...
The optimal number of cluster based on the k means clustering plot are eight cluster, this is inconsistent with the optimal number of cluster based on hierarchical clustering. For this type of clustering, the gap statistic stated that five was the optimal number of cluster to have. Five clusters was consistent acroos the different types of linkage methods, single, complete and average. Although the cluster dendrograms all look different the results from all of them are the same. 
...
plot(model.1.1, xvar="lambda", label=TRUE)
plot(model.1.1, xvar="dev", label=TRUE)
